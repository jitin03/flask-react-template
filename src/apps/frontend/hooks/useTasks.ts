...
import { TasksService, Task, CreateTaskRequest, UpdateTaskRequest } from '../services/tasks.service';

// Query keys
export const taskKeys = {
  all: ['tasks'] as const,
  lists: () => [...taskKeys.all, 'list'] as const,
  list: (accountId: string, filters?: Record<string, any>) => [...taskKeys.lists(), accountId, filters] as const,
  details: () => [...taskKeys.all, 'detail'] as const,
  detail: (accountId: string, taskId: string) => [...taskKeys.details(), accountId, taskId] as const,
};
...
export const useTask = (accountId: string, taskId: string, options?: { enabled?: boolean }) => {
  return useQuery({
...
export const useCreateTask = (accountId: string) => {
  ...
  return useMutation({
    mutationFn: (taskData: CreateTaskRequest) => TasksService.createTask(accountId, taskData),
...
export const useUpdateTask = (accountId: string, taskId: string) => {
  ...
  return useMutation({
    mutationFn: (taskData: UpdateTaskRequest) => TasksService.updateTask(accountId, taskId, taskData),
...
export const useDeleteTask = (accountId: string) => {
  ...
  return useMutation({
    ...
    onSuccess: (_, taskId) => {
      ...
      queryClient.removeQueries({ queryKey: taskKeys.detail(accountId, taskId) });
