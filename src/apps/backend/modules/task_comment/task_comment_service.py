...
class TaskCommentService:
    ...
    def create_comment(params: CreateTaskCommentParams) -> TaskComment:
        ...
        try:
            ...
            task = TaskService.get_task(params=GetTaskParams(task_id=params.task_id, account_id=params.account_id))
            if not task:
            ...
            Logger.error(
                message=f"Failed to verify task existence: {str(e)} (task_id={params.task_id}, account_id={params.account_id})"
            )
            raise TaskNotFoundForCommentError(params.task_id)
